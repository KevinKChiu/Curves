...
// ===YOUR CODE STARTS HERE===
	for (let k = 0; k < nSegment + 1; k++ ) {
		let t = k / nSegment;
		let pts = []; 
		let new_pts = [];
		for (let i = 0; i < controlPts.length; i++) {
			pts = [];
			for (let j = 0; j < controlPts.length - i - 1; j++) {
				let p1_x = 0;
				let p1_y = 0;
				let p2_x = 0;
				let p2_y = 0;
				...
				if (i === 0) {
					p1_x = controlPts[j].x;
					p1_y = controlPts[j].y;
					p2_x = controlPts[j + 1].x;
					p2_y = controlPts[j + 1].y;
					p_x = (1 - t) * p1_x + t * p2_x;
					p_y = (1 - t) * p1_y + t * p2_y;
				} else {
					p1_x = new_pts[j].x;
...
// ===YOUR CODE STARTS HERE===
	...
	function computeQtForBSpline(t, p0, p1, p2, p3) {
		let m1 = p0 * (-(t * t * t) + (3 * t * t) - (3 * t) + 1);
		let m2 = p1 * ((3 * t * t * t) - (6 * t * t) + 4);
		let m3 = p2 * ((-3 * t * t * t) + (3 * t * t) + (3 * t) + 1);
		let m4 = p3 * t * t * t;
		return (m1 + m2 + m3 + m4) / 6;
	}

	for (let h = 0; h < controlPts.length; h++) {
		let fourControlPts = [];
		if (!closedCurve) {
			if (h + 3 < controlPts.length) {
				fourControlPts[0] = controlPts[h];
				fourControlPts[1] = controlPts[h + 1];
				fourControlPts[2] = controlPts[h + 2];
				fourControlPts[3] = controlPts[h + 3];
			} else {
				break;
			}
		} else {
			if (h + 3 < controlPts.length) {
				...
				fourControlPts[2] = controlPts[h + 2];
				fourControlPts[3] = controlPts[h + 3];
			} else if (h + 2 < controlPts.length) {
				fourControlPts[0] = controlPts[h];
				fourControlPts[1] = controlPts[h + 1];
				fourControlPts[2] = controlPts[h + 2];
				fourControlPts[3] = controlPts[0];
			} else if (h + 1 < controlPts.length) {
				fourControlPts[0] = controlPts[h];
				fourControlPts[1] = controlPts[h + 1];
				fourControlPts[2] = controlPts[0];
				fourControlPts[3] = controlPts[1];
			} else {
				fourControlPts[0] = controlPts[h];
				fourControlPts[1] = controlPts[0];
				fourControlPts[2] = controlPts[1];
				fourControlPts[3] = controlPts[2];
			}
		}
		...
		for (let k = 0; k < nSegment + 1; k++) {
			...
			let p0x = fourControlPts[0].x;
			let p0y = fourControlPts[0].y;
			let p1x = fourControlPts[1].x;
			let p1y = fourControlPts[1].y;
			let p2x = fourControlPts[2].x;
			let p2y = fourControlPts[2].y;
			let p3x = fourControlPts[3].x;
			let p3y = fourControlPts[3].y;

			let px = computeQtForBSpline(t, p0x, p1x, p2x, p3x);
			let py = computeQtForBSpline(t, p0y, p1y, p2y, p3y);
...
// ===YOUR CODE STARTS HERE===
	...
	function computeQtForHermite(t, p0, p1, p0t, p1t) {
		let m1 = p0 * ((2 * t * t * t) - (3 * t * t) + 1);
		let m2 = p1 * ((-2 * t * t * t) + (3 * t * t));
		let m3 = 2 * p0t * ((t * t * t) - (2 * t * t) + t);
		let m4 = 2 * p1t * ((t * t * t) - (t * t));
		return m1 + m2 + m3 + m4;
	}
	...
	for (let i = 0; i < controlPts.length; i++) {
		...
		if (!closedCurve) {
			if (i + 1 < controlPts.length) {
				twoControlPts[0] = controlPts[i];
				twoControlPts[1] = controlPts[i + 1];
			} else {
				break;
			}
		} else {
		...
		for (let k = 0; k < nSegment + 1; k++) {
			...
			let p0x = twoControlPts[0].x;
			let p0y = twoControlPts[0].y;
			let p0xt = twoControlPts[0].dx;
			let p0yt = twoControlPts[0].dy;
			let p1x = twoControlPts[1].x;
			let p1y = twoControlPts[1].y;
			let p1xt = twoControlPts[1].dx;
			let p1yt = twoControlPts[1].dy;

			let px = computeQtForHermite(t, p0x, p1x, p0xt, p1xt);
			let py = computeQtForHermite(t, p0y, p1y, p0yt, p1yt);
